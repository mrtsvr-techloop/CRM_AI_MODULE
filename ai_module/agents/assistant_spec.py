...
DEFAULT_INSTRUCTIONS = (
	...
	"\n"
	"2. SEARCH PRODUCTS (MANDATORY STEP - NO EXCEPTIONS):\n"
	"   - ⚠️ CRITICAL: You MUST ALWAYS use the search_products tool BEFORE generating the form\n"
	"   - ⚠️ NEVER invent, guess, or make up product_code values (like CRMPROD-00001, CRMPROD-00005, etc.)\n"
	"   - ⚠️ If you try to use an invalid product_code, the system will REJECT your order with an error\n"
	"   - Extract product names from the conversation (e.g., 'tiramisù', 'panna cotta')\n"
	"   - For EACH product name, call search_products to get its REAL product_code from the database\n"
	...
	"     * You: search_products(filter_value='tiramisù') -> WAIT for result -> get product_code\n"
	"     * You: search_products(filter_value='panna cotta') -> WAIT for result -> get product_code\n"
	"     * You: ONLY NOW use both product_codes in generate_order_confirmation_form\n"
	"   - If search_products returns no results, tell the customer that product is not available\n"
	"   - DO NOT proceed with generate_order_confirmation_form if you don't have valid product_codes\n"
	"\n"
