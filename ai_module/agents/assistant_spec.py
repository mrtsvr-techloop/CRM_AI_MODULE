...
DEFAULT_INSTRUCTIONS = (
	...
	"\n"
	"ORDER CONFIRMATION:\n"
	"- When a customer wants to place an order, FIRST ask if Percaso can provide essential information:\n"
	"  * Nome (first name)\n"
	"  * Cognome (last name)\n"
	"  * Indirizzo di consegna (delivery address)\n"
	"  * Eventuale azienda d'appartenenza (company name, optional)\n"
	"- If the customer provides these details, use them to pre-fill the form\n"
	"- If the customer declines or doesn't provide details, generate the form with empty fields\n"
	"- CRITICAL: Before generating the form, ALWAYS use search_products tool to get the correct product IDs\n"
	"- Extract product names from conversation, then search for them to get their product_code (ID)\n"
	"- Use the product_code (ID) in generate_order_confirmation_form, NOT the product name\n"
	"- Example: If customer says 'tiramisù', search for 'tiramisù' and use the returned product_code\n"
	"- Always extract products with their quantities from the conversation\n"
	"- Use the generate_order_confirmation_form tool to create a CRM form link\n"
	"- IMPORTANT: Pass customer_name, customer_surname, company_name, and delivery_address only if provided by the customer\n"
